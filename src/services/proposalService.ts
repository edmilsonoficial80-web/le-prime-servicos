...
import type { ProfessionalProfile, Proposal, ServiceRequest } from '@/types';
...
export const createProposal = async (input: CreateProposalInput): Promise<string> => {
  ...
  if (!isFirebaseConfigured || !db) {
    demoStore.update((database) => {
      ...
      if (target) target.proposalsCount += 1;
    });
  } else {
...
export const observeRequestProposals = (
  requestId: string,
  callback: (list: Proposal[]) => void,
): (() => void) => {
  if (!isFirebaseConfigured || !db) {
    return demoSubscribe(
      (database) => sortByDateDesc(database.proposals.filter((p) => p.requestId === requestId)),
      callback,
    );
  }
  const q = query(
...
export const observeClientProposals = (clientId: string, callback: (list: Proposal[]) => void): (() => void) => {
  if (!isFirebaseConfigured || !db) {
    return demoSubscribe(
      (database) => sortByDateDesc(database.proposals.filter((p) => p.clientId === clientId)),
      callback,
    );
  }
  const q = query(
...
export const observeProfessionalProposals = (
  professionalId: string,
  callback: (list: Proposal[]) => void,
): (() => void) => {
  if (!isFirebaseConfigured || !db) {
    return demoSubscribe(
      (database) => sortByDateDesc(database.proposals.filter((p) => p.professionalId === professionalId)),
      callback,
    );
  }
  const q = query(
...
export const observeAllProposals = (callback: (list: Proposal[]) => void): (() => void) => {
  if (!isFirebaseConfigured || !db) {
  ...
  return onSnapshot(collection(db, COLLECTIONS.proposals), (snapshot) => {
    callback(sortByDateDesc(snapshot.docs.map((d) => ({ id: d.id, ...d.data() }) as Proposal)));
  });
};
  ...
  if (!isFirebaseConfigured || !db) {
    demoStore.update((database) => {
      database.proposals.forEach((p) => {
        if (p.requestId !== proposal.requestId) return;
    ...
    const snapshot = await getDocs(
      query(collection(db, COLLECTIONS.proposals), where('requestId', '==', proposal.requestId)),
    );
    await Promise.all(
      snapshot.docs.map((d) =>
        updateDoc(doc(db!, COLLECTIONS.proposals, d.id), {
...
export const rejectProposal = async (proposal: Proposal): Promise<void> => {
  if (!isFirebaseConfigured || !db) {
    demoStore.update((database) => {
...
export const acceptRequestDirectly = async (
  request: ServiceRequest,
  professional: ProfessionalProfile,
): Promise<void> => {
