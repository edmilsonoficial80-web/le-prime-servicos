...
const conversationKey = (a, b, requestId) => `conv_${[a, b].sort().join('_')}${requestId ? `_${requestId}` : ''}`;
/** Cria (ou reutiliza) a conversa entre dois utilizadores. */
export const ensureConversation = async (current, other, request) => {
...
export const observeConversations = (userId, callback) => {
    if (!isFirebaseConfigured || !db) {
        return demoSubscribe((database) => database.conversations
            .filter((c) => c.participantIds.includes(userId))
            .sort((a, b) => b.lastMessageAt - a.lastMessageAt), callback);
    }
    const q = query(collection(db, COLLECTIONS.conversations), where('participantIds', 'array-contains', userId), orderBy('lastMessageAt', 'desc'));
    return onSnapshot(q, (snapshot) => {
        callback(snapshot.docs.map((d) => ({ id: d.id, ...d.data() })));
    });
};
export const observeConversation = (conversationId, callback) => {
    if (!isFirebaseConfigured || !db) {
...
export const observeMessages = (conversationId, callback) => {
    if (!isFirebaseConfigured || !db) {
        return demoSubscribe((database) => database.messages
            .filter((m) => m.conversationId === conversationId)
            .sort((a, b) => a.createdAt - b.createdAt), callback);
    }
    const q = query(collection(db, COLLECTIONS.messages), where('conversationId', '==', conversationId), orderBy('createdAt', 'asc'));
    return onSnapshot(q, (snapshot) => {
        callback(snapshot.docs.map((d) => ({ id: d.id, ...d.data() })));
    });
};
export const sendMessage = async (conversation, senderId, text, imageFile) => {
...
export const countUnread = (conversations, userId) => conversations.reduce((total, conversation) => total + (conversation.unread?.[userId] ?? 0), 0);
/** Remove as conversas associadas a um pedido eliminado. */
export const deleteConversationsByRequest = async (requestId) => {
    if (!isFirebaseConfigured || !db) {
        demoStore.update((database) => {
    ...
    const snapshot = await getDocs(query(collection(db, COLLECTIONS.conversations), where('requestId', '==', requestId)));
