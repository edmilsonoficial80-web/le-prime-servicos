...
/**
 * Base de dados em memória (persistida em localStorage) usada quando o
 * Firebase não está configurado. Permite explorar a aplicação completa
 * sem qualquer credencial — ideal para demonstração e desenvolvimento.
 */
...
const face = (id: string) => `https://images.unsplash.com/${id}?w=256&h=256&q=80&auto=format&fit=crop&crop=faces`;
const work = (id: string) => `https://images.unsplash.com/${id}?w=800&q=80&auto=format&fit=crop`;

const seedProfessionals: ProfessionalProfile[] = [
  {
    uid: 'pro_1',
    role: 'professional',
    name: 'Miguel Andrade',
    ...
    description:
      'Eletricista certificado com foco em instalações residenciais, quadros elétricos e domótica. Trabalho limpo, garantia de 2 anos.',
    experienceYears: 12,
    rating: 4.9,
    reviewsCount: 148,
    completedJobs: 213,
  ...
  },
  {
    ...
    description:
      'Coordeno remodelações completas de cozinhas e casas de banho. Orçamento detalhado e prazos cumpridos.',
    experienceYears: 9,
    rating: 4.8,
    reviewsCount: 96,
    completedJobs: 121,
...
const seedRequests: ServiceRequest[] = [
  {
    ...
    description:
      'O quadro é antigo e dispara com frequência. Preciso de substituição completa com diferencial e certificação.',
...
class DemoStore {
  ...
  update(mutator: (db: DemoDatabase) => void): void {
    mutator(this.state);
    this.persist();
...
export const demoSubscribe = <T>(selector: (db: DemoDatabase) => T, callback: (value: T) => void): (() => void) => {
