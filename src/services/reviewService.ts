...
import type { AppUser, ProfessionalProfile, Review, ServiceRequest } from '@/types';
...
export const createReview = async (input: CreateReviewInput): Promise<void> => {
  ...
  if (!request.professionalId) throw new Error('Este pedido ainda não tem profissional atribuído.');
...
export const observeProfessionalReviews = (
  professionalId: string,
  callback: (list: Review[]) => void,
): (() => void) => {
  if (!isFirebaseConfigured || !db) {
    return demoSubscribe(
      (database) => sortByDateDesc(database.reviews.filter((r) => r.professionalId === professionalId)),
      callback,
    );
  }
  const q = query(
