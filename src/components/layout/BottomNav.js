...
import { Briefcase, ClipboardList, Heart, LayoutDashboard, MessageCircle, Plus, Search, ShieldCheck, User, Wallet, } from 'lucide-react';
...
export const BottomNav = () => {
    ...
    if (!user)
        return null;
    if (/^\/(conversas\/|avaliar\/|cliente\/novo-pedido)/.test(pathname))
        return null;
    let items = [];
    if (isClient) {
        items = [
    ...
    return (_jsx("nav", { className: "z-30 border-t border-ink-100 bg-white/95 backdrop-blur safe-bottom", children: _jsx("ul", { className: "flex items-stretch justify-around px-1 pt-1.5 pb-1", children: items.map(({ to, label, icon: Icon, badge, highlight }) => (_jsx("li", { className: "flex-1", children: _jsx(NavLink, { to: to, end: to === '/profissional' || to === '/admin', className: ({ isActive }) => cn('relative flex flex-col items-center gap-1 rounded-xl px-1 py-1.5 text-[10px] font-semibold transition-colors', isActive ? 'text-ink-900' : 'text-ink-400'), children: ({ isActive }) => (_jsxs(_Fragment, { children: [_jsxs("span", { className: cn('relative flex h-9 w-9 items-center justify-center rounded-xl transition-all', highlight
                                    ? 'h-10 w-10 -translate-y-1 bg-brand-yellow text-ink-900 shadow-glow'
                                    : isActive
                                        ? 'bg-ink-900 text-brand-yellow'
                                        : 'text-ink-400'), children: [_jsx(Icon, { size: highlight ? 21 : 19, strokeWidth: 2.2 }), Boolean(badge) && (_jsx("span", { className: "absolute -right-0.5 -top-0.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-red-500 px-1 text-[9px] font-bold text-white", children: badge > 9 ? '9+' : badge }))] }), _jsx("span", { className: "truncate", children: label })] })) }) }, to))) }) }));
};
